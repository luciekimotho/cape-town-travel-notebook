...
function FormActions({ label, busy, onDelete }: { label: string; busy: boolean; onDelete?: () => void }) {
  return <div className="form-actions">{onDelete && <button className="delete-icon" type="button" onClick={onDelete} aria-label={`Delete ${label}`} title={`Delete ${label}`}><LineIcon name="trash"/></button>}<button className="save-icon" disabled={busy} type="submit" aria-label={`Save ${label}`} title={`Save ${label}`}><LineIcon name="save"/></button></div>
}
...
function ActivityForm({ data, item, place, expense, defaultDayId, fixedTemplate, busy, onSave, onDelete }: {
  ...
  return <form className="form-card activity-form" onSubmit={submit}>
    <label className="field">Name *<input name="name" required maxLength={90} defaultValue={place?.name ?? fixedTemplate?.name}/></label>
...
function Itinerary({ data, commit, busy, onOpen }: SectionProps & { onOpen: (itemId: string, parentId?: string) => void }) {
  ...
  return <section className="page">
    <div className="section-row"><h2>Itinerary</h2><button className="icon-button add-button" aria-label="Add activity" onClick={() => setEditorOpen(true)}>+</button></div>
...
function ItineraryCard({ item, data, onOpen }: { item: ItineraryItem; data: AppData; onOpen: (itemId: string, parentId?: string) => void }) {
  ...
  const heading = <button className="activity-card" onClick={() => onOpen(item.id)}><span className="thumb"><PlaceScene name={place.name}/></span><span className="card-copy"><h3>{place.name}</h3>{children.length > 0 && <small>{children.length} stops · {children.filter(child => child.visited).length} stamped</small>}{item.time && <small>{item.time}</small>}{expense && <small className="cost-pill">{money(expense.amount, expense.currency)}</small>}</span>{stamp ? <span className="mini-stamp"><TravelStamp name={place.name} date={stamp.visitDate}/></span> : <span className="chevron" aria-hidden="true">›</span>}</button>
...
function RouteRow({ item, data, compact, onOpen }: { item: ItineraryItem; data: AppData; compact?: boolean; onOpen: () => void }) {
  ...
  return <button className={`route-row ${compact ? 'compact' : ''}`} style={{ '--place-color': colorForKind(kind) } as React.CSSProperties} onClick={onOpen}><span className={`stop-number ${stamp ? 'is-stamped' : ''}`}>{stamp ? <TravelStamp name={place.name} date={stamp.visitDate}/> : <MarkerIcon kind={kind}/>}</span><span className="stop-copy">{place.name}{item.notes && <small>{item.notes}</small>}</span><span className="chevron" aria-hidden="true">›</span></button>
}
...
function ActivityDetail({ route, data, commit, busy, onBack, onOpenChild }: { route: DetailRoute; data: AppData; commit: SectionProps['commit']; busy: boolean; onBack: () => void; onOpenChild: (id: string) => void }) {
  ...
  return <>
    ...
    <main className="detail-main">
      <div className="hero"><PlaceScene name={place.name}/>{stamp && <div className={`hero-stamp ${justStamped ? 'stamp-pop' : ''}`}><TravelStamp name={place.name} date={stamp.visitDate}/></div>}</div>
      ...
      {stamp ? <div className="stamped-line"><button onClick={toggleStamp}>Undo stamp</button></div> : <button className="stamp-action" onClick={toggleStamp}><LineIcon name="moments"/>Stamp this visit</button>}
      {(place.address || place.googleMapsUrl || item.notes) && <section className="detail-section specifics">{place.address && <p>{place.address}</p>}{item.notes && <p>{item.notes}</p>}{place.googleMapsUrl && <a href={place.googleMapsUrl} target="_blank" rel="noreferrer">Open in Google Maps</a>}</section>}
      ...
      <section className="detail-section"><div className="detail-heading"><h2>Moments</h2>{stamp && <button className="text-action" onClick={() => setPhotoOpen(true)}>{photo ? 'Edit' : '+ Add'}</button>}</div>{photo ? <><PhotoImage photo={photo} alt={photo.caption || place.name} className="memory-image"/>{photo.caption && <p className="caption">{photo.caption}</p>}</> : stamp ? <button className="photo-placeholder" onClick={() => setPhotoOpen(true)}><LineIcon name="camera"/>Add a photo</button> : <p className="small-label">Stamp your visit to add a photo.</p>}</section>
      <section className="detail-section"><div className="detail-heading"><h2>Cost</h2><button className="text-action" onClick={() => setEditorOpen(true)}>{expense ? 'Edit' : '+ Add'}</button></div>{expense && <div className="expense-row"><strong>{money(expense.amount,expense.currency)}</strong></div>}</section>
    </main>
...
function Checklist({ data, commit, busy }: SectionProps) {
  ...
  const categories=[...new Set(items.map(item=>item.category))].sort((a,b)=>a==='Shopping'?-1:b==='Shopping'?1:a.localeCompare(b))
  return <section className="page"><div className="section-row"><h2>Checklist</h2><button className="icon-button add-button" aria-label="Add reminder" onClick={() => {setEditing(undefined);setEditorOpen(true)}}>+</button></div><div className="day-summary"><h3>Preparation</h3><span>{items.filter(item=>item.completed).length}/{items.length}</span></div>{categories.map(category=><section className="checklist-section" key={category}><p className="list-label">{category}</p><div className="plain-list">{items.filter(item=>item.category===category).map(item=><div className={`plain-row checklist-row ${item.completed?'checked':''}`} key={item.id}><button className="check-toggle" aria-label={`${item.completed?'Uncheck':'Complete'} ${item.title}`} aria-pressed={item.completed} onClick={()=>commit(()=>db.checklist.update(item.id,{completed:!item.completed,updatedAt:timestamp()}))}><span className="check-box">{item.completed?'✓':''}</span></button><button className="copy row-open" onClick={()=>{setEditing(item);setEditorOpen(true)}}><strong>{item.title}</strong>{item.dueDate&&<small>{formatDate(item.dueDate)}</small>}</button></div>)}</div></section>)}
    <Sheet open={editorOpen} title={editing?'Edit reminder':'Add reminder'} onClose={close}><form className="form-card" onSubmit={submit}><label className="field">Reminder *<input name="title" required defaultValue={editing?.title}/></label><div className="fields-two"><label className="field">Category<input name="category" defaultValue={editing?.category ?? 'Planning'}/></label><label className="field">Due date<input name="dueDate" type="date" defaultValue={editing?.dueDate}/></label></div><label className="field">Note<textarea name="note" defaultValue={editing?.note}/></label><FormActions label="reminder" busy={busy} onDelete={editing?remove:undefined}/></form></Sheet>
  </section>
}
